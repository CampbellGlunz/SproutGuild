        ...
        var parameters = deepExtend(true, baseParameters, {
          find: {
            $or: [
              ...
              {categories: {$elemMatch: {name: {$regex: terms.query, $options: 'i'}}}}
            ]

          }
        });
        if (!Posts.find({}, parameters).fetch()[0]){
          throwError(i18n.t('The item you searched returned no results.'));
          Router.go('/');
          $('.search-field').focus();
