...
if(Meteor.isClient){
  ...
  Router.onAfterAction(filters.resetScroll, {except:['posts_top', 'posts_new', 'posts_best', 'posts_pending', 'posts_category', 'all-users', 'posts_digest']});
...
PostsDigestController = FastRender.RouteController.extend({
  ...
  waitOn: function() {
    ...
    return [
      Meteor.subscribe('postsListInf', limit),
  ...
  data: function() {
    ...
    var newestPost =Posts.findOne({},{sort: {submitted:-1}});
    var days = [];
    var oldestPost = Posts.findOne({},{sort: {submitted:1}});
    
    var oldestDate = moment(oldestPost.submitted);
    var newestDate = moment(newestPost.submitted).add('days',1);

    for (var m = oldestDate; m.isBefore(newestDate); m.add('days', 1)) {
      var n = moment(m);
      var o = moment(m);
      var start = n.startOf('day').valueOf();
      var end = o.endOf('day').valueOf();
      
      days.push({
        dayposts: Posts.find({ submitted: {$gte: start , $lt: end }}, {sort: {baseScore: -1}}),
        day: m.format('dddd, MMMM Do'),
        today: m.format('D')
    });
    }
    var posts = Posts.find({}, {sort: {submitted: -1}, limit: this.limit()});
    var hasMore = posts.count() === this.limit();
...
UserPageController = FastRender.RouteController.extend({
  waitOn: function() {
    ...
    return [
      ...
      Meteor.subscribe('postsListPerUser', this._terms, this.params._idOrSlug),
    ];
  },
  data: function() {
    ...
    if(typeof findById !== "undefined"){
      ...
      var user = (typeof findById == "undefined") ? findBySlug : findById;
      var posts = Posts.find({userId: user._id});
      return {
        user: user,
        posts: posts,
      };
...
Router.map(function() {
  ...
  this.route('post_edit', {
    ...
    waitOn: function () {
      return [
      Meteor.subscribe('singlePost', this.params._id),
      Meteor.subscribe('allUsers', this.params_id)
      ];
    },
    data: function() {
      return {
